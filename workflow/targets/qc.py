...
def qc_outputs(results_dir: Path, samples: List[str]) -> List[str]:
    """
    ...
    # Fastp reports
    outputs += expand(
        f"{results_dir}/qc/{{sample}}/trimmed/{{sample}}_fastp.html",
        sample=samples,
    )

    # BWA alignment to contaminants
    outputs += expand(
        f"{results_dir}/qc/{{sample}}/decontamination/{{sample}}_aligned_contaminants.sam",
        sample=samples,
    )

    # Contaminant read IDs
    outputs += expand(
        f"{results_dir}/qc/{{sample}}/decontamination/{{sample}}_contaminants_id.txt",
        sample=samples,
    )

    # Cleaned reads (only specifying R1 here, but R2 will be generated in the same step)
    outputs += expand(
        f"{results_dir}/qc/{{sample}}/decontamination/{{sample}}_R1.cleaned.fastq.gz",
        sample=samples,
    )
