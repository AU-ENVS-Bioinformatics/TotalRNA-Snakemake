rule bbduk_ssu:
    ...
    input:
        rRNA_r1=f"{RNA_CLASSIFIED_DIR}/{{sample}}/rRNA/{{sample}}_rRNA_1.fastq.gz",
        rRNA_r2=f"{RNA_CLASSIFIED_DIR}/{{sample}}/rRNA/{{sample}}_rRNA_2.fastq.gz",
    ...
    output:
        ...
        rRNA_non_ssu_r1=f"{RNA_CLASSIFIED_DIR}/{{sample}}/non_SSU/{{sample}}_non_SSU_1.fastq.gz",
        rRNA_non_ssu_r2=f"{RNA_CLASSIFIED_DIR}/{{sample}}/non_SSU/{{sample}}_non_SSU_2.fastq.gz",
    log:
        stdout=f"{RNA_INTERMEDIATE_DIR}/{{sample}}/logs/bbduk_ssu.log"
    benchmark:
        f"{RNA_INTERMEDIATE_DIR}/{{sample}}/benchmarks/bbduk_ssu.txt"
    params:
